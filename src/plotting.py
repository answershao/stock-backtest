...
def plot_expected_return_frame(
    ...
    *,
    ...
    ax.plot(
        frame["date"],
        frame["mean_reversion_return_3y"],
        label=labels["mean_reversion"],
        linewidth=2,
    )
    ax.plot(frame["date"], frame["consensus_cagr_3y"], label=labels["consensus_cagr"], linewidth=2)
    ax.plot(
        frame["date"], frame["expected_return_3y"], label=labels["expected_return"], linewidth=2.4
    )
    ax2.plot(
        frame["date"],
        frame["close"],
    ...
    ax.set_title(
        labels["title"].format(stock=display_name, start_date=start_date, end_date=end_date)
    )
